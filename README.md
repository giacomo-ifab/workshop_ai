...
- In fondo alla scheda, **"Scarica PDF"**: il PDF si compone con le primitive testuali di jsPDF (`src/lib/useCasePdf.ts`), non fotografando il DOM, così va su più pagine e il testo resta selezionabile. Il layout è posizionato a mano, quindi valgono due regole: `y` è sempre la prossima baseline utile e ogni blocco chiede spazio con `ensure()` prima di disegnare. I titoli di sezione stanno dentro una fascia piena (una riga di separazione vicino alla baseline finirebbe dentro le lettere), le scelte singole e multiple vanno su due colonne etichetta/valore, i campi descrittivi a tutta larghezza con rientro sporgente sui punti elenco, e ogni pagina ha il piede con nome, sessione e numero di pagina. Le lineette lunghe e gli apici tipografici vengono convertiti: i font standard di jsPDF sono WinAnsi e li renderebbero come buchi.
